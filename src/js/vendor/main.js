...
if ( ! functionExists( 'pageEffectOnUnload' ) ) {
	ish.pageEffectOnUnload = function() {
		...
		$('.ish-blurred a').click(function(event) {
			...
			if ( !( event.defaultPrevented || event.isDefaultPrevented() || event.ctrlKey || event.shiftKey || event.metaKey || event.which == 2) ){
				...
				if ( 0 > actTargetHref.indexOf('#') && 0 > actTargetHref.indexOf('http') && false == $( actTarget ).parent().hasClass('ish-has-submenu') ) {
...
if ( ! functionExists( 'prepareNavigationActiveItem' ) ) {
	ish.prepareNavigationActiveItem = function() {
		var activeItem = $('.ish-active-item');
...
if ( ! functionExists( 'showHideNavigation' ) ) {
	ish.showHideNavigation = function() {
		...
		var ishNavigation = $('.ish-navigation');
		var setOpacity = function(displayVal){
			ishNavigation.css( 'opacity', displayVal );
		};

		$('.ish-menu-container a, .ish-nav-close').click(function( event ) {
			event.preventDefault();
...
if ( ! functionExists( 'clickNavigation' ) ) {
	ish.clickNavigation = function() {
		$('.ish-navigation li a').click(function() {
			var thisEl = $(this);
			//$('.ish-navigation .ish-active-item').removeClass('ish-active-item');
...
if ( ! functionExists( 'preventNavWithSubmenu' ) ) {
	ish.preventNavWithSubmenu = function() {
		$('.ish-has-submenu > a').click(function(event) {
			event.preventDefault();
		})
	}
}
...
if ( ! functionExists( 'backToTopButton' ) ) {
	ish.backToTopButton = function() {
		$('.ish-back-to-top > a').click(function(event) {
			event.preventDefault();
...
if ( ! functionExists( 'decorationHeight' ) ) {
	ish.decorationHeight = function() {
		...
		function applyDecorHeight() {
			var decorHeight = Math.round( 0.177 * $(window).width() ) + 10 ; // 0.177 = tan 10° // 10 = margin 10px
			$('.ish-decor-container').css('height', decorHeight + 'px');
			$('.ish-no-content header').css('margin-bottom', '-' + decorHeight + 'px');
			$('#map').css('margin', '-' + decorHeight + 'px 0');
		}
	}
}
...
if ( ! functionExists( 'activatePortfolio' ) ) {
	ish.activatePortfolio = function() {
		...
		var portfolio = $('.ish-pflo-gal.ish-2col');
			...
			var checkPortfolio = function() {
				...
				if ( 768 <= windowWidth ) {
					...
					if ( ishMacyActive ) {
						...
						// Reposition captions
						var ishPortfolio = $('.ish-pflo-gal.ish-scroll-anim.ish-2col');
...
if ( ! functionExists( 'activateBlog' ) ) {
	ish.activateBlog = function() {
		...
		var blog = $('.ish-blog-masonry.ish-2col');
		if ( blog.length > 0 ) {
...
if ( ! functionExists( 'activateContentParallax' ) ) {
	ish.activateContentParallax = function() {
		var parallaxSection = $('.ish-content-parallax');
...
if ( ! functionExists( 'validateForm' ) ) {
	ish.validateForm = function() {
		...
		// Validate form
		$('form input[type="text"].required').on('input', function() {
			verifyText( $(this) );
		});
		...
		$('form input[type="url"]').on('input', function() {
			verifyUrl( $(this) );
		});

		$('form textarea.required').keyup(function() {
			verifyMessage( $(this) );
		});
...
if ( ! functionExists( 'initMap' ) ) {
	ish.initMap = function() {
		...
		var myLatLng = {lat: myLat, lng: myLng}; // {lat: 36.5215692, lng: -4.9643864};
		var styleArray = [
			{
				featureType: "all",
				stylers: [
...
if ( ! functionExists( 'activateAccordion' ) ) {
	ish.activateAccordion = function() {
		...
		$('.ish-sc-accordion > dt > a').click(function() {
...
if ( ! functionExists( 'activateToggle' ) ) {
	ish.activateToggle = function() {
		...
		$('.ish-sc-toggle > dt > a').click(function() {
			var thisEl = $(this).parent();
...
if ( ! functionExists( 'activateTabs' ) ) {
	ish.activateTabs = function() {
		...
		$('.ish-tabs-menu a').click(function(event) {
			event.preventDefault();
			$(this).parent().addClass('ish-active-item');
			$(this).parent().siblings().removeClass('ish-active-item');
			var tab = $(this).attr('href');
			$('.ish-tab-content').not(tab).css('display', 'none');
			$(tab).fadeIn();
		});
...
if ( ! functionExists( 'arrangeWidgetRecentProjects' ) ) {
	ish.arrangeWidgetRecentProjects = function() {
		...
		function setImageTiles() {
			var widgetRecentProjectLi = $('.ish-widget-recent-projects li');
			var tileWidth = Math.floor( $('.ish-sidebar').width() / 3 );
